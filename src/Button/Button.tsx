...
const Background: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Sausage
    const sausageRadius = 0.1;
    const sausageLength = 1;
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (intersects.length > 0) {
        ...
        // Move sausage towards mouse position
        sausage.position.x += (targetPosition.x - sausage.position.x) * 0.1;
        sausage.position.z += (targetPosition.z - sausage.position.z) * 0.1;

        // Roll the sausage
        const rollAngle = Math.atan2(targetPosition.z - sausage.position.z, targetPosition.x - sausage.position.x);
        sausage.rotation.y = rollAngle;
        
        // Add a slight tilt in the direction of movement
        sausage.rotation.x = (targetPosition.z - sausage.position.z) * 0.2;
        sausage.rotation.z = -(targetPosition.x - sausage.position.x) * 0.2;
      }
    };
